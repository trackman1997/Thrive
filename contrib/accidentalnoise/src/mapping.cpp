...
namespace anl
{
   void map2D(int seamlessmode, CArray2Dd &a, CImplicitModuleBase &m, SMappingRanges &ranges, double z)
   {
        ...
        for(x=0; x<w; ++x)
        {
            for(y=0; y<h; ++y)
            {
                double p=static_cast<double>(x) / static_cast<double>(w);
                double q=static_cast<double>(y) / static_cast<double>(h);
                double r;
    ...
    void map3D(int seamlessmode, CArray3Dd &a, CImplicitModuleBase &m, SMappingRanges &ranges)
    {
        ...
        for(x=0; x<w; ++x)
        {
            for(y=0; y<h; ++y)
            {
                ...
                a.set(static_cast<unsigned int>(x), static_cast<unsigned int>(y),
                        static_cast<unsigned int>(z), val);
                }
            }
        }
    }
